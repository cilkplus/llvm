...
LValue CodeGenFunction::EmitMaterializeTemporaryExpr(
  ...
  CGCilkSpawnInfo *Info = dyn_cast_or_null<CGCilkSpawnInfo>(CapturedStmtInfo);
  bool IsReceiverDecl = Info && Info->isReceiverDecl(M->getExtendingDecl());

  if (IsReceiverDecl) {
    assert(Info->getReceiverTmp() && "receiver temporary expected");
    Object = Info->getReceiverTmp();
  } else
    Object = createReferenceTemporary(*this, M, E);
  ...
  // Receiver cleanup is emitted in the caller and not in the helper.
  if (!IsReceiverDecl)
    pushTemporaryCleanup(*this, M, E, Object);
