...
AttrResult Sema::ActOnPragmaSIMDLinear(SourceLocation LinearLoc,
  ...
  return AttrResult(::new (Context) SIMDLinearAttr(
      LinearLoc, Context, const_cast<Expr **>(Exprs.data()), Exprs.size(), 0));
}
...
AttrResult Sema::ActOnPragmaSIMDPrivate(SourceLocation ClauseLoc,
  ...
  case SIMD_Private:
    Clause = ::new (Context)
        SIMDPrivateAttr(ClauseLoc, Context, Exprs.data(), Exprs.size(), 0);
    break;
  case SIMD_LastPrivate:
    Clause = ::new (Context)
        SIMDLastPrivateAttr(ClauseLoc, Context, Exprs.data(), Exprs.size(), 0);
    break;
  case SIMD_FirstPrivate:
    Clause = ::new (Context)
        SIMDFirstPrivateAttr(ClauseLoc, Context, Exprs.data(), Exprs.size(), 0);
    break;
  }
...
Sema::ActOnPragmaSIMDReduction(SourceLocation ReductionLoc,
  ...
  SIMDReductionAttr *ReductionAttr = ::new SIMDReductionAttr(
      ReductionLoc, Context, VarList.data(), VarList.size(), 0);
...
private:
  ...
  Stmt *SimplifyExpr(Stmt *S) {
    ...
    DeclStmts.push_back(Res.get());
    ExprResult ERes = SemaRef.BuildDeclRefExpr(TempVD, E->getType(), VK_LValue,
                                               SourceLocation());
    if (!E->isGLValue()) {
      ERes = SemaRef.DefaultLvalueConversion(ERes.get());
    }
    return ERes.get();
  }
...
ExprResult Sema::ActOnCEANIndexExpr(Scope *S, Expr *Base, Expr *LowerBound,
  ...
  if (ArgsDep)
    return new (Context) CEANIndexExpr(
  ...
  if (!LowerBound)
    LowerBound = ActOnIntegerConstant(ColonLoc1, 0).get();
  else {
  ...
  if (!Length) {
    ...
    if (const ConstantArrayType *ConstArrType =
      ...
      Length = ActOnIntegerConstant(
          ColonLoc1, ConstArrType->getSize().getZExtValue()).get();
    else if (const VariableArrayType *VarArrType =
                 dyn_cast<VariableArrayType>(ArrType))
      Length = VarArrType->getSizeExpr();
    Length = CreateBuiltinBinOp(ColonLoc1, BO_Sub, Length, LowerBound).get();
    if (!Length)
      return ExprError();
  } else {
    CEANRankCalculator RankCalc(*this);
  ...
  if (!Stride)
    Stride = ActOnIntegerConstant(SourceLocation(), 1).get();
  else {
    CEANRankCalculator RankCalc(*this);
  ...
  return new (Context)
      CEANIndexExpr(Base, LowerBoundRes.get(), ColonLoc1, LengthRes.get(),
                    ColonLoc2, StrideRes.get(), ResType);
}
  ...
  if ((Rank > 0 || MaxAllowedRank > 0) && !CEANLevelCounter) {
    ...
    for (CEANRankCalculator::ExprListVec::iterator I = CEANExprs.begin(),
      ...
      if (Length) {
        ...
        Vars.push_back(DS);
        Expr *DRE = BuildDeclRefExpr(VD, VD->getType(), VK_LValue,
                                     SourceLocation()).get();
        Incs.push_back(
            CreateBuiltinUnaryOp(SourceLocation(), UO_PreInc, DRE).get());
        for (II = I->begin(), EE = I->end(); II != EE; ++II) {
          if (*II) {
            QualType QTy = (*II)->getType();
            ExprResult Res = DefaultLvalueConversion(DRE);
            Res = PerformImplicitConversion(Res.get(), QTy, AA_Casting);
            Res = CreateBuiltinBinOp(SourceLocation(), BO_Mul,
                                     (*II)->getStride(), Res.get());
            Res = CreateBuiltinBinOp(SourceLocation(), BO_Add,
                                     (*II)->getLowerBound(), Res.get());
            (*II)->setIndexExpr(Res.get());
          }
        }
        if (!BIEndFound) {
          for (CEANRankCalculator::BuiltinExprList::iterator BII = BI->begin(),
            ...
            if (*BII) {
              ...
              ExprResult Res = DefaultLvalueConversion(DRE);
              Res = PerformImplicitConversion(Res.get(), Ret->getType(),
                                              AA_Casting);
              (*BII)->setReturnExpr(Res.get());
            }
          }
        }
      }
...
ExprResult Sema::ActOnCEANBuiltinExpr(Scope *S, SourceLocation StartLoc,
  ...
  case CEANBuiltinExpr::ImplicitIndex: {
    ...
    ImplicitIndex = PerformImplicitConversion(
        ImplicitIndex.get(), Context.getIntPtrType(), AA_Converting);
    RankedExpr = 0;
  } break;
  ...
  if (Rank > 0) {
        ...
        for (II = I->begin(); II != EE; ++II) {
          if (*II) {
            ...
            IndExpr = PerformImplicitConversion(
                Res.get(), Context.getIntPtrType(), AA_Converting);
    ...
    case CEANBuiltinExpr::ReduceAdd: {
      ...
      InitExpr =
          PerformImplicitConversion(InitExpr.get(), ResType, AA_Casting);
    } break;
    case CEANBuiltinExpr::ReduceMul: {
      if (!ResType->isScalarType() && !ResType->isAnyComplexType()) {
        Diag(RankedExpr->getExprLoc(), diag::err_cean_not_scalar_or_complex)
    ...
    case CEANBuiltinExpr::ReduceMax: {
      ...
      if (ResType->hasSignedIntegerRepresentation()) {
        ...
        InitExpr = FloatingLiteral::Create(Context, Val, true, ResType,
                                                 SourceLocation());
    ...
    if (CKind != CEANBuiltinExpr::ReduceMutating) {
      ...
      Return = ReturnRes.get();
      Expr *RetVal = DefaultLvalueConversion(Return).get();
      ExprResult CalcExpr;
      switch (CKind) {
      case CEANBuiltinExpr::ReduceAdd:
        CalcExpr =
      ...
      case CEANBuiltinExpr::ReduceAllZero: {
        ...
        CalcExpr = ActOnConditionalOp(
            ...
            BuildUnaryOp(S, RankedExpr->getExprLoc(), UO_LNot,
                         ImpCastExprToType(Res.get(),
                                           Context.getLogicalOperationType(),
                                           CK).get()).get(),
            RetVal, ActOnIntegerConstant(RankedExpr->getExprLoc(), 0).get());
      } break;
      case CEANBuiltinExpr::ReduceAllNonZero: {
        CastKind CK = ScalarTypeToBooleanCastKind(RankedExpr->getType());
        ExprResult Res = DefaultLvalueConversion(RankedExpr);
        CalcExpr = ActOnConditionalOp(
            RankedExpr->getExprLoc(), RankedExpr->getExprLoc(),
            ImpCastExprToType(Res.get(), Context.getLogicalOperationType(), CK)
                .get(),
            RetVal, ActOnIntegerConstant(RankedExpr->getExprLoc(), 0).get());
      } break;
      case CEANBuiltinExpr::ReduceAnyZero: {
        ...
        CalcExpr = ActOnConditionalOp(
            ...
            BuildUnaryOp(S, RankedExpr->getExprLoc(), UO_LNot,
                         ImpCastExprToType(Res.get(),
                                           Context.getLogicalOperationType(),
                                           CK).get()).get(),
            ActOnIntegerConstant(RankedExpr->getExprLoc(), 1).get(), RetVal);
      } break;
      case CEANBuiltinExpr::ReduceAnyNonZero: {
        CastKind CK = ScalarTypeToBooleanCastKind(RankedExpr->getType());
        ExprResult Res = DefaultLvalueConversion(RankedExpr);
      ...
      case CEANBuiltinExpr::ReduceMaxIndex: {
        ExprResult AssignInd =
            BuildBinOp(S, SourceLocation(), BO_Assign, IndDRE, IndExpr.get());
        ExprResult ResExpr = CreateBuiltinBinOp(SourceLocation(), BO_Comma,
                                                AssignInd.get(), RankedExpr);
        CalcExpr = ActOnConditionalOp(
            RankedExpr->getExprLoc(), RankedExpr->getExprLoc(),
            BuildBinOp(S, RankedExpr->getExprLoc(), BO_GT, RetVal, RankedExpr)
                .get(),
      ...
      case CEANBuiltinExpr::Reduce: {
        ...
        if (ULE && ULE->getNamingClass() &&
          ...
          if (MemberExpr.isInvalid())
            return ExprError();
          CallArgs.clear();
          CallArgs.push_back(Res.get());
          Expr *Callee = MemberExpr.get();
          CalcExpr =
      ...
      if (CallExpr.isInvalid())
        return ExprError();
      Body =
          ImpCastExprToType(CallExpr.get(), Context.VoidTy, CK_ToVoid).get();
    }
    Res = CEANBuiltinExpr::Create(Context, StartLoc, RParenLoc, Rank, CKind,
                                  Args, Lengths, Vars, Incs, InitStmt, Body,
                                  Return, ResType);
  } else if (CKind == CEANBuiltinExpr::ImplicitIndex) {
    Res = CEANBuiltinExpr::Create(Context, StartLoc, RParenLoc, 0,
                                  CEANBuiltinExpr::ImplicitIndex, Args, Lengths,
                                  Vars, Incs, 0, 0, ImplicitIndex.get(),
