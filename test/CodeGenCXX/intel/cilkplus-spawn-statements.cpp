...
// CHECK-CILK7: call {{.*}} @{{.*}}i
// CHECK-CILK7: call void @{{.*}}Foo
// CHECK-CILK7-NEXT: call void @__cilk_helper_prologue
// CHECK-CILK7-NEXT: {{call|invoke}} {{.*}} @{{.*}}Foo

// CHECK-CILK7: store
// CHECK-CILK7-NEXT: call void @{{.*}}Foo

//-----------------------------------------------------------------------------

// Test for handling lambda functions correctly
void test8() {
  // CHECK-CILK8: define void @{{.*}}test8
...
// CHECK-CILK8-NEXT: {{call|invoke}} i32 @{{.*}}test8vENK3$_0clE3Foo

// CHECK-CILK8: call void @{{.*}}Foo
